#!/usr/bin/env bash
REPO_NAME="cf-security-entitlement"
NAME="terraform-provider-cfsecurity"
OS=""
...
provider_path="${HOME}/.terraform.d/providers/terraform-provider-cfsecurity"
grep -Fxq "providers {" ~/.terraformrc &> /dev/null
if [[ $? != 0 ]]; then
    cat <<EOF >> ~/.terraformrc
providers {
    cfsecurity = "$provider_path"
}
EOF
else
    grep -Fxq "cfsecurity" ~/.terraformrc &> /dev/null
    if [[ $? == 0 ]]; then
        echo "${NAME}-${VERSION} has been installed."
        exit 0
    fi
    awk -v provider_path="$provider_path" '/providers {/ { print; printf "    cfsecurity = \""; printf provider_path; print "\""; next }1' ~/.terraformrc > /tmp/.terraformrc
    mv /tmp/.terraformrc ~/
fi
