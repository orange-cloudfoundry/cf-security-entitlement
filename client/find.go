...
func (c Client) ListSecGroups(query ...ccv3.Query) ([]SecurityGroup, error) {
	...
	_, _, err := c.session.V3().MakeListRequest(ccv3.RequestParams{
...
func (c Client) GetSecGroupByName(name string) (SecurityGroup, error) {
	...
	res, httpres, err := c.session.V3().MakeRequestSendReceiveRaw(
		"GET",
		fmt.Sprintf("%s/v3/security_groups?names=%s", c.session.V3().CloudControllerURL, name),
		http.Header{},
		nil,
	)
	if err != nil {
...
func (c Client) GetSecGroupByGuid(guid string) (SecurityGroup, error) {
	...
	res, httpres, err := c.session.V3().MakeRequestSendReceiveRaw(
		"GET",
		fmt.Sprintf("%s/v3/security_groups/%s", c.session.V3().CloudControllerURL, guid),
		http.Header{},
		nil,
	)
	if err != nil {
...
func (c Client) GetOrgByGuid(guid string) (Organization, error) {
	...
	res, httpres, err := c.session.V3().MakeRequestSendReceiveRaw(
		"GET",
		fmt.Sprintf("%s/v3/organizations/%s", c.session.V3().CloudControllerURL, guid),
		http.Header{},
		nil,
	)
	if err != nil {
...
func (c Client) GetSpaceByGuid(guid string) (Space, error) {
	...
	res, httpres, err := c.session.V3().MakeRequestSendReceiveRaw(
		"GET",
		fmt.Sprintf("%s/v3/spaces/%s", c.session.V3().CloudControllerURL, guid),
		http.Header{},
		nil,
	)
	if err != nil {
...
func (c Client) GetSecGroupRelationships(SecGroup SecurityGroup) (SecurityGroup, error) {
	...
	for _, spaceGuid := range SecGroup.Resources[0].Relationships.Running_spaces.Data {
		...
		Org, _, _ := c.session.V3().GetOrganization(Space.Relationships["organization"].GUID)
...
func (c Client) ListUserManagedOrgs(userGuid string) (UserRoles, error) {
	...
	res, httpres, err := c.session.V3().MakeRequestSendReceiveRaw(
		"GET",
		fmt.Sprintf("%s/v3/roles?user_guids=%s", c.session.V3().CloudControllerURL, userGuid),
		http.Header{},
		nil,
	)
	if err != nil {
...
func (c Client) ListOrgManagers(orgGuid string) (User, error) {
	...
	res, httpres, err := c.session.V3().MakeRequestSendReceiveRaw(
		"GET",
		fmt.Sprintf("%s/v3/roles?organization_guids=%s", c.session.V3().CloudControllerURL, orgGuid),
		http.Header{},
		nil,
	)
	if err != nil {
